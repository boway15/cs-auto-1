# mail-guide-ai v1.7.0 + v1.7.1 同批上线说明（请运维按序执行）

> 研发发版文档 · 同批从 **v1.6.0 → v1.7.1**（含 1.7.0 附件修复 + 1.7.1 取消定时草稿）  
> 模板见 `docs/ops-release-notice-template.md`

**发版类型**：v1.6.0 → **v1.7.1**（同批合并 1.7.0 + 1.7.1，只跑 **一次** 发版脚本）  
**代码分支**：GitLab `main`  
**提交号**：`28c2a4267b4e63bb89856776db8506f0b3626f2f`（tip；功能主体 `810f5bb` + `3b0afec`）  
**建议顺序**：同步代码 → 后端脚本（Functions + migration + vault/cron）→ 环境变量（跳过）→ 前端（跳过）→ 验收
...
| 材料 | 用途 |
|------|------|
| 无新增密钥 | 本批不改 `.env.functions` 必填项；现网已有配置即可 |
...
**操作**：本次**跳过**（勿因本批单独 rebuild 前端镜像）。

---

### 第 5 步：验收

**目的**：确认 Functions、migration、cron 均已生效。
...
| `ops.backend_release_migrations` | 出现 `20260714120000_unschedule_auto_draft_cron.sql`，`release_version` 为 `v1.7.1` |
...
| 3 | 环境变量 | 本次跳过 |
| 4 | 页面更新 | 本次跳过 |
| 5 | 确认 OK | Functions 健康 + `auto-draft` 不存在 + 附件抽查 + `post-deploy-verify.sh` |
