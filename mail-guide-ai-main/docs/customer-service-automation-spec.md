...
| 运营告警（关联/拦截） | **首次 + 末次**各一封邮件（`ops_alerts` 独立幂等键）；**已取消**原 compensating **2h 定时**提醒。 |
...
| 订单补偿任务 | **最多补偿 20 次**（收信首次查单另计，总约 21 次）；**每 20 分钟**调度（`next_run_at` 步长 20 分钟，`*/20` cron）。 |
...
- 默认 **`max_retries = 20`**（迁移 `20260520120000_automation_12h_twenty_compensation_retries.sql`）。
- 每次未查到订单时，**`next_run_at` 推迟 20 分钟**（与 `run-compensation-tasks` 的 `*/20` cron 一致）。
- 仅对 **`received_at` 在 12 小时内**的邮件创建/执行补偿（`isEmailWithinCustomerAutomationAge`）。

关联成功后，后续拦截、草稿等逻辑依赖 **linkedOrders.length > 0**。

---

## 4. 两套时间线（客户自动回复 vs 运营告警）

二者 **独立**，需同时判断：

| 类型 | 时间条件 | 用途 |
|------|----------|------|
| **客户自动回复** | `now - received_at ≤ 12h` | 仅在此窗口内给客户发自动模板；**超过 12h 一律不发**。 |
| **运营告警邮件** | 与 **12h 客户窗无直接对应** | 自动关联/拦截在**首次失败**与**末次终态**各发一封（见 **§7**）；`not_provided` **不发**关联类告警。 |

**说明**：

- 客户 12h 与运营告警 **互不替代**（例如仍可对客户不发信，但对内部发「首次未关联」）。
- **`not_provided`**：**不**发订单关联类运营告警。
- **取消/改地址** 且邮件 **已超过 12h**：**禁止**对客户自动回复（即使开关打开）。
...
|------|----------------|-----------|----------|
| **未提供订单号**（无法关联） | 总开关开 **且** ≤12h **且** 槽一模板（`ar_missing_order`）`auto_send` 与意图勾选满足：发索要单号等模板；否则不发 | **不**调用拦截 | **不**发内部预警（已确认） |
| **有单号且关联成功** | 一般不先发「缺单号」模板 | **调用** `risk-intercept`（本地 hold；**不**调 Shopify） | 自动拦截失败：**首次 + 末次**运营邮件（§7） |
| **有单号但未关联成功**（compensating） | **不对客户**发「缺单号」模板 | **不**拦截（直至关联成功） | 创建补偿任务时 **首次**运营邮件；用尽/策略终止时 **末次**（各一封，见 **§7**） |

**特别强调**：**超过 12h** 的取消/改地址邮件 **不**对客户自动回复（即使开关打开）。

### 6.2 破损（`damaged`）、缺陷（`defect`）、描述不符（`description_mismatch`）

| 条件 | 行为 |
|------|------|
| **可配置首封窗口**（每条双槽模板 `reply_templates.auto_reply_first_contact_days`：0=不限，3/7/15/30 天；**按实际触发的槽**各自校验；同发件人在该窗口内无其它邮件）+ **信息不完整**（缺单号或缺附件等，与 `missing_elements` 一致） | 命中自动模板回复时，仍须满足 **§5.2 通用条件**（含 ≤12h、总开关） |
...
| **补偿** | 若该次未成功（进入待补偿/重试类状态），再**最多 20 次**自动补偿；两次补偿间隔 **20 分钟**（`next_compensation_at` / `COMPENSATION_STEP_MS`）。 |
| **终态** | **收信 1 次 + 补偿 20 次**仍不成功 → **拦截失败**（与 `risk_intercept_logs.status` 等对齐）。 |
| **成功** | 任意一次成功 → 成功终态，不再排补偿。 |
| **Cron** | `retry-risk-intercept-compensation` 为 **`*/20 * * * *`**（job 名仍可能带 `hourly-at-45` 历史遗留）；每轮处理 `next_compensation_at` 已到且 `compensation_attempts_done < 20` 的记录。 |

#### 6.4.3 自动拦截准入（不满足则不拦、不排补偿）

1. **未关联本地订单**且**工作流判定未提供订单号** → **不自动拦截**（与 §6.1「未提供订单号不调用拦截」一致方向；未关联但**已**提供单号时，可按实现走凭邮件单号等路径）。
2. **发件时间**：以邮件头发送时间为准，相对**当前执行时刻**已超过 **12 小时** → **不自动拦截**、**不**进入自动补偿。
   - **发件时间不可解析**：须在实现中约定一种**保守**策略（建议倾向不自动拦截），并在迁移/代码注释中写死。

#### 6.4.4 人工不受限

- **开关**、**12 小时发件时间**均**不**限制人工在工作台发起的拦截/解除。
...
通过 `ops_alerts` + `createAlertAndNotify` 发邮件；**独立** `idempotency_key` 保证同一链路最多 **首次、末次各一封**。

### 7.1 自动订单关联

| 阶段 | 触发 | `kind` | 实现 |
|------|------|--------|------|
| **首次** | 收信 `associateOrders` 本地/OMS 均未命中，`order_compensation_tasks` upsert 成功 | `auto_association_first` | `process-email` → `notifyAutoAssociationFirstFailure` |
| **末次** | `retry_count >= max_retries`（默认 20）仍无订单 | `auto_association_final` | `run-compensation-tasks`，`reason: max_retries` |
| **末次（策略）** | `manual_unlink` 或发件超 **12h** 停止补偿 | `auto_association_final` | `reason: manual_unlink` / `stale_email` |

标题示例：`[首次] 订单未关联：{order_no}`、`[末次] 订单关联失败：{order_no}`。
...
- **`schedule-compensating-alerts`** 及 cron `compensating-alerts-every-30min`（原 2h compensating 扫描）已由本节 **首次/末次** 替代；部署脚本会 `unschedule` 旧 job。

预警内容建议包含：`email_id`、订单号、`order_no` / `log_id`、`business_intent`、失败原因摘要。
...
2. **取消/改地址**：无单号 + 开关 + ≤12h → 客户自动模板；compensating → **首次/末次**运营邮件（§7）；**不对客户**发缺单号模板。
3. **总开关 + 分场景开关**：**Supabase secrets**；所有 `sendTemplateReply` 路径前统一校验 **12h**。
4. **运营告警**：`ops-notify` + `automation-*-alerts.ts`；关联/拦截各 **首次 + 末次**；无 `schedule-compensating-alerts` cron。
5. **自动草稿**：拦截/关联后 `status` 仍满足「不排除」候选条件。
6. **草稿管线**：消费 `ai_language` / `ai_sentiment`。
7. **Dify DSL**：`email-analysis.yml` 输出与后端字段对齐。
8. **补偿任务**：`max_retries` 默认 **20**，重试间隔 **20 分钟**（迁移 `20260520120000_automation_12h_twenty_compensation_retries.sql` + Edge）。
9. **风控**：**不调用 Shopify**；ERP 直连按 `erp-api-requirements.md` 迭代。
10. **自动风控拦截**：§6.4 开关、准入（无单号且未关联 / 超 12h 发件时间）、收信 1 次 + 补偿 20 次、`*/20` cron、开关关终态；人工始终可拦。
...
- [ ] 任意客户自动回复：`received_at` 超过 12h → 不发。
- [ ] 取消/改地址 + 超 12h → 不发客户信。
- [ ] `not_provided` → 无内部 compensating 类预警。
- [ ] compensating：创建补偿任务 → `auto_association_first` + 邮件；用尽 20 次 → `auto_association_final` + 再发一封。
- [ ] 自动拦截：首次 `retrying` → `auto_failed_first`；末次 `failed` → `auto_failed_final`（与首次各一封）。
- [ ] 有单号且关联成功 + 取消/改地址 → 触发拦截（本地 hold，不调 Shopify）。
- [ ] 破损/缺陷/描述不符 + 首封 + 信息不完整 + ≤12h + 开关 → 可发模板。
- [ ] `ai_language` / `ai_sentiment` 落库；异常时语言为 `en`。
- [ ] 自动草稿：1～24h、pending/processing、无草稿、龄≥1h；1～6h Dify、6～24h 本地。
- [ ] 人工草稿仅本地。
- [ ] 补偿任务最多 **20** 次失败告警，**20 分钟** 间隔。
...
- [ ] **自动风控拦截（§6.4）**：开关关 → 无自动拦、无补偿；开关开 → 无单号且未关联 / 发件超 12h **不**自动拦；收信失败后最多 20 次补偿、间隔 20min；用尽失败终态；补偿 cron 为 **`*/20`**；开关关闭后进行中记录收敛为拦截失败；**人工**不受开关与 12h 限制。
...
- **Edge**：`process-email`、`run-compensation-tasks`（20min 步长、20 次）、`risk-intercept`（不调 Shopify）、**`retry-risk-intercept-compensation`**、`_shared/ops-notify.ts`、`_shared/automation-alert-keys.ts`、`_shared/automation-intercept-alerts.ts`、`_shared/automation-association-alerts.ts`、`_shared/auto-risk-intercept-policy.ts`（12h 窗）、**`RiskLogs`（自动拦截开关）**、`_shared/draft.ts`、`generate-draft` / `schedule-draft-generation`（`schedule-compensating-alerts` 已废弃）
